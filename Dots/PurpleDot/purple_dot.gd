...
func _ready():                                                                                          #When it spawns in
	add_to_group("PURPLES")                                                                                #Adds to group "REDS"
...
func _draw():                                                                                           #Overrides original draw func
	draw_circle(Vector2(0, 0), globals.RADIUS, Color(5, 0, 7.5, 0.75))                                          #Draws the dot
...
func _physics_process(_delta):
	...
	for a in REDS.size():
		var RED = REDS[0]
		if RED != self:
			var dis: Vector2 = DOTMOVEMENTSYSTEM.GETDISTANCE(global_position, RED.global_position)
			...
			if dis1.y < dis.y:
				...
				dis.y = dis2.y
			var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(4, 0)
	...
	for a in BLUES.size():
		var BLUE = BLUES[0]
		var dis: Vector2 = DOTMOVEMENTSYSTEM.GETDISTANCE(global_position, BLUE.global_position)
		var dis1: Vector2 = DOTMOVEMENTSYSTEM.GETDISTANCE(global_position - globals.SCREENSIZE, BLUE.global_position)
		var dis2: Vector2 = DOTMOVEMENTSYSTEM.GETDISTANCE(global_position + globals.SCREENSIZE, BLUE.global_position)
		if dis1.x < dis.x:
			dis.x = dis1.x
		elif dis2.x < dis.x:
			dis.x = dis2.x
		if dis1.y < dis.y:
			...
			dis.y = dis2.y
		var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(4, 1)
		var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
		VELOCITY += ATTRACTIONTOTAL * globals.SENSITIVITY
		BLUES.pop_at(0)
	...
	for a in GREENS.size():
		...
		if dis1.y < dis.y:
			...
			dis.y = dis2.y
		var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(4, 2)
		var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
		VELOCITY += ATTRACTIONTOTAL * globals.SENSITIVITY
		GREENS.pop_at(0)
	...
	for a in ORANGES.size():
		...
		if dis1.y < dis.y:
			...
			dis.y = dis2.y
		var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(4, 3)
		var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
	...
	for a in PURPLES.size():
		...
		if PURPLE != self:
			...
			if dis1.y < dis.y:
				...
				dis.y = dis2.y
			var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(4, 4)
			var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
			linear_velocity += ATTRACTIONTOTAL * globals.SENSITIVITY
		PURPLES.pop_at(0)
	...
	for a in YELLOWS.size():
		...
		if dis1.y < dis.y:
			...
			dis.y = dis2.y
		var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(4, 5)
		var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
		linear_velocity += ATTRACTIONTOTAL * globals.SENSITIVITY
		YELLOWS.pop_at(0)
