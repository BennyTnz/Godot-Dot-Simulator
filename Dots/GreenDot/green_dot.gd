...
func _draw():                                                                                           #Overrides original draw func
	draw_circle(Vector2(0, 0), globals.RADIUS, Color(0, 0.75, 0, 1))                                          #Draws the dot
...
func _physics_process(_delta):
	...
	for a in GREENS.size():
		...
		if GREEN != self:
			...
			if dis1.y < dis.y:
				...
				dis.y = dis2.y
			var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(2, 2)
			var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
			VELOCITY += ATTRACTIONTOTAL * globals.SENSITIVITY
		GREENS.pop_at(0)
	...
	for a in ORANGES.size():
		...
		if dis1.y < dis.y:
			...
			dis.y = dis2.y
		var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(2, 3)
		var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
	...
	for a in PURPLES.size():
		...
		if dis1.y < dis.y:
			...
			dis.y = dis2.y
		var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(2, 4)
		var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
		linear_velocity += ATTRACTIONTOTAL * globals.SENSITIVITY
		PURPLES.pop_at(0)
	...
	for a in YELLOWS.size():
		...
		if dis1.y < dis.y:
			...
			dis.y = dis2.y
		var COLOURTOCOLOURVALUE = ATTRACTIONCALCULATOR.GETATTRACTION(2, 5)
		var ATTRACTIONTOTAL = MOVES.GETATTRACTION(dis, COLOURTOCOLOURVALUE)
		linear_velocity += ATTRACTIONTOTAL * globals.SENSITIVITY
		YELLOWS.pop_at(0)
